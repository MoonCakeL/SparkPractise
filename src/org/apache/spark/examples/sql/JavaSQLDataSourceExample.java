...
public class JavaSQLDataSourceExample {
  ...
  private static void runBasicDataSourceExample(SparkSession spark) {
    ...
    // $example on:manual_load_options_csv$
	  Dataset<Row> busCSV = spark.read().format("csv")
			                            .option("inferSchema", "true") //推断Schema
			                            .option("header", "true")
			                            .load("src/main/resources/201404bus.csv");
  ...
  private static void runParquetSchemaMergingExample(SparkSession spark) {
    ...
    //hdfs上的目录结构"data/test_table/key=1"
    ...
    // Create another DataFrame in a new partition directory,
	  //从一个新的hdfs目录下（分区）创建一个dataframe然后添加一个新的列"data/test_table/key=2"
