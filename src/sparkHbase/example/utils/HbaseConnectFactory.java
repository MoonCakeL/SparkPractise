...
public class HbaseConnectFactory {
	...
	private static final Logger logger = LoggerFactory.getLogger(HbaseConnectFactory.class);
	...
	private  HbaseConnectFactory() {
		if (CLUSTER_NAME == null){
			...
			try {
				hbaseConnect = ConnectionFactory.createConnection(getHBaseConfiguration());
	...
	private static HbaseConnectFactory hbaseConnectFactory = new HbaseConnectFactory();
	
	//1.static静态类型项目启动就直接在内存中存在了，(直接完成“饿汉模式”)类加载的时候就创建对象
	//外部调用getInstance方法获取实例，再.getHbaseConnect()获取连接
	public static HbaseConnectFactory getInstance(){
